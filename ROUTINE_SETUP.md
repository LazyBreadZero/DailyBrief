...
3. Gather widely, then cut hard (see SOURCING + SELECTION in briefing_system_prompt.md): lead from primary/tier-one sources (FT, Economist, Bloomberg, Reuters, filings, journals, regulator + funding databases), use trade-blogs only to discover a story then verify/cite the primary one. Run the open "what matters most today in human-experience tech" query plus a rotating secondary lens (capital / clinical-regulatory / platforms / science / policy). No general AI-industry drift.
4. Rank candidates by significance and apply the 80/20 bar: print ONLY what clears it. No quota per beat; a running thread is a story only if it MOVED (never print countdowns or "no news"). Length tracks the signal — a one-story edition on a quiet day is correct, not thin. Write the self-contained HTML briefing (sized to the news, short days allowed) to briefings/<DATE_ISO>.html.
