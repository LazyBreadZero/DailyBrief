...
- **Is BCI's commercial structure resolved into one axis, or does it still run on three?** (Ed 10-12, consolidated) Hardware/regulatory (jurisdiction-driven), algorithmic (academic-timeline, Ed 10), and software-defined scope (Ed 11 Neuralink) were already three axes; Ed 12's electrode-count story adds a fourth — signal resolution — on its own iteration timeline, independent of the other three. Has Angelo's map of "what determines BCI's commercial future" kept pace, or is he still defending a one- or two-axis frame?
- **Is "spatial computing" one market or two?** (Ed 11-12) Snap's $2,500 Specs vs. Android XR's sub-$500 audio glasses (Ed 11) is the AR/XR analogue of BCI's axis-bifurcation. Ed 12's Seoul showroom (Korea Metaverse Festival, Ray-Ban Meta / Galaxy XR / Vision Pro side by side) shows the comparison is now global, not just US/EU — and raises the possibility of a fourth ecosystem entrant. Has Angelo picked which market he's translating for the rooms he's in, or is the picture now wider than two?
- **What does Angelo believe that AWE's 400 speakers won't say?** (Ed 10, last call) AWE's "I, Spatial" theme is his positioning language, now the industry's — and, per Ed 12, the institution is now also writing XR's history (Hall of Fame) and canon (Art Festival) under that same theme. AWE opens Monday — this question becomes retrospective after Friday's edition.
- **Has Angelo been citing unverified "world's first" claims?** (new, Ed 12) Edition 3 called NeuroXess "the world's first commercial brain implant"; this week, credible outlets (including MIT Technology Review) make the identical claim about a different company, NeuracleTech's NEO. Whether these refer to the same event is unresolved. What is Angelo's actual verification standard for an NMPA claim vs. an FDA/CE-mark claim — and has it been the same standard?
- **Is the XR/BCI convergence institutionally real, or only Angelo's frame?** (new, Ed 12) AWE's 2026 XR Hall of Fame inducted 8 people — none from neurotech/BCI — in the same week it expanded its academic track. If the institution that increasingly defines "spatial computing" hasn't yet written BCI into XR's history, is "post-screen human interface" a frame Angelo is ahead of, or one that isn't shared by the field that would need to ratify it?

### Positions the evidence now implies (and whether his view has moved)
- **"Consent" beats "governance" not on the merits but on enforcement.** Ed 11's Meta episode is the clearest evidence yet: disclosure violations get fixed because someone can sue or report on them; "should this capability exist at all" questions get a code deletion and silence because no one can compel more. For BCI/neurotech, where the dormant-capability pattern already exists architecturally (Wheelchair Control, Precision Neuroscience/Medtronic, Neuralink), this predicts that any future neural-data equivalent of Stella would be "resolved" the same way — unless Angelo's audience (regulators, capital allocators) builds the missing enforcement mechanism before it's needed.
- **BCI's commercial structure may now run on four axes, not one, two, or three.** Hardware/regulatory (jurisdiction-driven), algorithmic (academic-timeline, Ed 10), software-defined scope (Ed 11 Neuralink "any brain region"), and now signal resolution (Ed 12's 1,000→3,000 electrode increase, its own iteration timeline). Each edition since Ed 10 has added an axis the previous framing didn't have room for — which is itself the pattern, not just the content.
- **AR/XR's bifurcation is now a global pattern, not a US/EU one.** Snap's $2,500 vs. Android XR's sub-$500 (Ed 11) parallels BCI's axis split; Ed 12's Seoul showroom shows the same three-ecosystem comparison being staged by a government, in Asia, simultaneously — "spatial computing" may be concealing not two markets but a wider, multi-region field that Edition 11's framing didn't anticipate.
- **Spatial computing's near-term form factor is settled for the mass-market band**: glasses-as-accessory, launching this autumn (Ed 10) — but Ed 11 shows a second, separate prosumer/enterprise AR-display band (Snap, and Apple circa 2029) exists in parallel, not as a future stage of the same product.
- **The convergence thesis runs ahead of the institutions that would ratify it.** Ed 12: AWE's Hall of Fame and Art Festival arrived alongside its Research Poster Track, but the Hall of Fame's history of XR contains no neurotech. If Angelo's "post-screen human interface" frame is correct, the field's own institutions haven't yet caught up to it — which changes what "translator" means: translating a convergence that exists, vs. arguing for one that doesn't yet have institutional standing.

### Blind spots he keeps circling
- Reaching for the **softer or more familiar word** (consent vs governance; jurisdiction vs algorithm vs software-defined scope vs signal resolution) when the harder framing is where the argument lives. Each edition since Ed 10 has added an axis to a frame Angelo had treated as complete.
- Treating **single-axis or single-market framings as complete** — "the BCI sector" as a fixed cast (Ed 9-10), "jurisdiction is the variable" as the whole picture (Ed 10-11), "spatial computing" as one (or two) markets (Ed 11-12). Ed 12 adds a new instance of the same pattern at the institutional level: treating "the field" as a single, settled convergence when its own history-writing institutions (AWE's Hall of Fame) don't yet reflect it.
- **New (Ed 12):** Citing "world's first" claims, especially from opaque regulatory environments (NMPA), without flagging that the claim itself may not be unique or verifiable — a credibility-grading gap in Angelo's own past citations, not just in new sources.
...
---
*Last updated: Edition 12 — 12 June 2026*
