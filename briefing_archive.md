...
- **Is "jurisdiction is the variable" complete, or only half the picture?** (new, Ed 10) Yale's neural-manifold result and Merge Labs' AI-layer thesis (Ed 4) imply a second axis — algorithm-to-brain fit — that runs on an academic timeline, not a regulatory one. A jurisdiction can clear hardware while the "does it work for this person" question remains open. Has Angelo named what the second variable is, or is the regulatory frame standing in for the whole picture because it's the part that makes news?
- **What does Angelo believe that AWE's 400 speakers won't say?** AWE's "I, Spatial: Humans Empowered by Spatial AI" is his positioning language, adopted by the world's largest XR conference. The framing has become the industry's. The live question is what substantive position Angelo holds beyond the consensus — and AWE opens in five days, the last natural deadline for this question to matter before the conference's content is locked.
- **Consent or governance?** Ten editions running. Edition 10 sharpened the test case considerably: Stella's dormant facial-recognition pipeline (50M phones, never disclosed, never activated) is not a disclosure failure about a specific event — it's a "should this exist at all, awaiting a remote flag" question that "consent" may not even have vocabulary for. Which steelman does Angelo reach for first, and what does that reveal about which primitive he's actually defending?
- **Is the commercial BCI layer being built by incumbents or startups?** Medtronic × Precision Neuroscience (Ed 9) and Apple's Wheelchair Control (Ed 9) are two data points outside the startup-race frame; Yale's algorithm-axis result (Ed 10) potentially adds a third kind of actor entirely — university labs — to a field Angelo has read as a three-way startup/incumbent contest. Has his map of "who could plausibly win" expanded to match?
- **The Science Corp thesis, not the contact:** ten editions outstanding. Whether Angelo holds a thesis on retinal BCI's commercial path sharp enough to bring to a conversation — the CE mark, if it arrives, creates a commercial narrative gap Max Hodak has not filled.

### Positions the evidence now implies (and whether his view has moved)
- **BCI's commercial structure may bifurcate into two races on two timelines.** A hardware/regulatory race (Neuralink, Synchron, Science Corp, Precision Neuroscience — jurisdiction-driven, the race this briefing has tracked) and a software/algorithmic race (Merge Labs, Yale and other academic labs — hardware-agnostic, publication-driven). Angelo's view should move from "which startup wins which jurisdiction's approval" to "which race actually determines whether BCI works for the people who get it — and is that the race anyone is covering?"
- **BCI's commercial structure is more complex than the startup-regulatory-race narrative.** Medtronic × Precision Neuroscience and Apple Wheelchair Control both suggest incumbent-integration paths exist and are being executed — now joined by Yale's evidence that university labs, with no commercial stake, may be producing the most consequential results. The "who could win" map has gained at least two dimensions (incumbent integration; academic algorithm research) beyond the startup race.
- **Spatial computing's near-term form factor is settled: glasses-as-accessory, launching this autumn.** Edition 10 dated this concretely — Samsung, XREAL, Warby Parker, Gentle Monster all shipping fall 2026. The live question is no longer *which form factor* or *when* but *whose software layer makes it indispensable*.
- **"Jurisdiction is the variable" in BCI** should now be held with an explicit caveat: it is a complete answer to "who gets approved first" and an incomplete answer to "whose device actually helps the most people" — per Edition 10's algorithm-axis finding. Whether Angelo's framing has absorbed this caveat is the live question above.

### Blind spots he keeps circling
- Reaching for the **softer word or the more familiar frame** (consent vs governance; jurisdiction vs algorithm) when the harder framing is where the argument actually lives — and where the people worth convincing will push back.
- Treating **"the BCI sector" as a fixed cast of companies** (Neuralink, Synchron, Science Corp, Precision Neuroscience, Merge Labs) when the most consequential developments increasingly come from outside that cast — Apple (Ed 9), Medtronic (Ed 9), and now a Yale neuroscience department (Ed 10) with no product and no funding round. The boundary of "who counts" may need redrawing before the thesis can be complete.
